//https://replit.com/join/xgwnlfxhyf-sylviacr
#include <iostream>
#include <vector>
#include <algorithm>
#include <fstream>
...
void rc4(char choice){
    ...
    if (choice == 'c'){
      ...
      for(auto i : cifrado){
          ...
          file << static_cast<char>(i); // Guardar en archivo
      }
      file.close();
      cout << endl;
      ...
      ofstream fileDecrypted("decrypted.txt");
      char i;
      //leer del archivo cifrado
      vector<int> cifrado;
      while (fileEncrypted.get(i)){
        cifrado.push_back(i);
      }
      ...
      for (auto i  : mensaje_descifrado){
          cout << static_cast<char>(i); // Convertir a caracteres imprimibles
          fileDecrypted << static_cast<char>(i); // Guardar en archivo 
      }
      cout << endl;
    }
...
int main() {
    char choice;
    cout << "Que desea realizar: encriptacion (c) o desencriptacion (d): ";
    while (cin >> choice && (choice == 'c' || choice == 'd')){
      rc4(choice);
    }
    return 0;
}